...
export function MyPagePanel({ onBack }: MyPagePanelProps = {}) {
  ...
  if (!isAuthenticated || !user) {
    return (
      <div className="space-y-4 font-mono">
        <div className="mx-auto w-full max-w-[560px] rounded-2xl border border-white/10 bg-[#0f0f0f] p-4 md:p-5">
          <p className="mb-3 text-[11px] tracking-[0.16em] text-[#8ea8c7] uppercase">로그인 / 회원가입</p>
          <AccountAuthPanel />
        </div>
      </div>
    );
  ...
  return (
    <div className="font-mono h-full min-h-0">
      <div className="rounded-[28px] border border-white/10 bg-[#0d0d0d] p-3 md:p-4 h-full min-h-0">
        <div className="grid grid-cols-1 lg:grid-cols-[300px_minmax(0,1fr)] gap-4 md:gap-5 h-full min-h-0">
      ...
      {adminComposer && (
        <div className="fixed inset-0 z-[140] flex items-center justify-center p-2 md:p-4">
          ...
          <div className="relative w-[98vw] h-[94vh] md:w-[min(1200px,95vw)] md:h-[min(860px,90vh)] rounded-xl md:rounded-3xl border border-white/15 bg-[#0d0d0d] overflow-hidden shadow-[0_40px_120px_rgba(0,0,0,0.7)]">
                ...
                <button
                  ...
                  className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors ${
                    adminComposer === 'products'
                      ? 'bg-[#7bb8ff]/20 border border-[#7bb8ff]/50 text-[#e8f3ff] shadow-[inset_0_0_0_1px_rgba(123,184,255,0.25)]'
                ...
                <button
                  ...
                  className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors ${
                    adminComposer === 'collections'
                      ? 'bg-[#00ffd1]/20 border border-[#00ffd1]/50 text-[#e8fff9] shadow-[inset_0_0_0_1px_rgba(0,255,209,0.25)]'
                      : 'bg-[#1a1a1a] border border-white/15 text-[#bdbdbd] hover:bg-[#222]'
                  }`}
                >
                  컬렉션 게시물
                </button>
                <p className="hidden md:block text-[11px] text-[#9eb8cc] ml-2 truncate">
            ...
            <div className="h-[calc(100%-64px)] border-t border-white/5">
              <iframe
